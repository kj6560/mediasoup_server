...
class Conference extends BaseModel
{
    ...
    {
        if ($user_id != null) {
            $query = "select conference.*,u.name from conference inner join users u on  u.id in(conference.conference_for) where conference.organisation=$organisation  and conference.is_deleted !=1 ";
        } else {
            $query = "select conference.*,u.name from conference inner join users u on u.id in(conference.conference_for) where conference.organisation=$organisation  and conference.is_deleted !=1 ";
        }
        echo $query;
